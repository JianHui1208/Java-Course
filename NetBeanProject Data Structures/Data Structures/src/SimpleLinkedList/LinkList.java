...
public class LinkList { //contains only one data item: a reference to the first link on the list. This reference is called first
    Link first;         //ref to first link on list
    public LinkList(){  //constructor
        first = null;   //on links on list yet
    }
    ...
    public Link deleteFirst(){      //delete first item(assumes list not empty)
        ...
        first = first.next;     //deyele it: first --> old next
    ...
    public Link delete(int key){        //delete link with given key(assumes non-empty list)
        ...
        while(current.iData != key){
            if(current.next == null){
                ...
                previous = current;     //go to link
                current = current.next;
            }
        }                               //found it
        if(current == first){           //it first link
