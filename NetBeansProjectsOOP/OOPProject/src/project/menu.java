...
public class menu {
    ...
    public double Control(String Name, String number, String email, int count) {
        double subtotal = 0;
        double TotalPrice= 0;
        int x = 1;
        ...
        boolean ControlKeyword = getKeyword();
        if(ControlKeyword){
            TotalPrice = subtotal * 80/100;
        }else{
            TotalPrice = subtotal;
        }
        ...
        for (int i = 1; i < count; i++) {
            if(OrderName[i] != null && FoodQuantity[i] != 0){
                System.out.println(x + ". " + OrderName[i] + " * " + FoodQuantity[i]);
                MSG[i] = x + ". " + OrderName[i] + " * " + FoodQuantity[i];
                x++;
            }
        }
        System.out.println("\nTotal price is: RM" + TotalPrice);
        this.alltotal = TotalPrice;
        
        try {
            double totalPrice = this.alltotal;
            String setName = Name;
        ...
        for (int j = 0; j < count; j++) {
            if(OrderName[j] != null && FoodQuantity[j] != 0){
                OrderName[j] = null;
                FoodQuantity[j] = 0;
                subTotal[j] = 0;
            }
    ...
    public void printReceipt(String Name){
        String[] totalordername = new String[99];
        String setName = "";
        String setNumber = "";
        String setEmail = "";
        String price = "";
        try {
            ...
            while (data.hasNext()) {
                ...
                for(int i = 0;i < totalordername.length;i++){
                    if(data.hasNext()!= false){
                        totalordername[i] = data.nextLine();
                    }
                }
            }
            System.out.println("\n---------------- RECEIPT ----------------\n");
            ...
            for(int j =0;j<totalordername.length;j++){
                if(totalordername[j]!=null){
                    System.out.println(totalordername[j]);
                }
            }
        } catch (FileNotFoundException e) {
            System.out.println("File no found");
    ...
    public String Payment(double payment, int count) {
        String MSG = "";
        double totoalprice = this.alltotal;
        if (payment < totoalprice) {
            System.out.println("\nInconsistent Amount!!! Pls Enter angin.");
            payment = input.nextInt();
        }
        double Change = payment - this.alltotal;
        if (Change > 0) {
